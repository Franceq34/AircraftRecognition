...
First, import the latest LWRP using package manager. You may need to expand the dropdown in the package manager to get the absolute latest version.

Once LWRP is installed, import the WeatherMaker/Demo/LWRP/WeatherMakerLWRP.unitypackage.txt for LWRP demo assets, Assets -> Import Package -> Custom, change extension to *.* to find it.

Having imported LWRP, there should be a scripting define of UNITY_LWRP defined. If not, please add it to the scripting defines. This will turn on LWRP in Weather Maker.

Now, set your render pipeline asset to the render pipeline asset extracted from the WeatherMakerLWRP.unitypackage. This setting can be found in project settinsg -> graphics right at the top.

There are a few shaders where grab pass needs to be commented out in order for them to show up properly in LWRP. Search and replace all files and replace 'GrabPass { "_CameraOpaqueTexture" }' with '//GrabPass { "_CameraOpaqueTexture" }'.

It is always best to restart Unity at this point so it can reload everything from scratch.

If you decide to stop using LWRP and go back to standard pipeline, please remove the LWRP package and then remove the UNITY_LWRP scripting define. This will turn off LWRP for Weather Maker.

When going back to standard pipeline, you must once more search and replace shaders to re-enable grab pass. Search and replace all files and replace '//GrabPass { "_CameraOpaqueTexture" }' with 'GrabPass { "_CameraOpaqueTexture" }'.

That's all there is to it! Water LWRP will be coming in a future update.
