...
fixed3 ComputeDirectionalLightFog(float3 rayOrigin, float3 rayDir, float rayLength, float fogFactor, float2 screenUV, bool volumetric)
{
	...
	float scatter = CalculateFogDirectionalLightScatter(rayDir, lightDir, fogFactor, _WeatherMakerDirLightPower[0].x, _WeatherMakerFogDensityScatter);
	fixed3 baseScatter = (fogFactor * ((dirLightColor * cloudShadow3) + (dirLightColor * scatter * cloudShadowSquared)));
	fixed rayLerp = min(1.0, lightDir.y * 3.0);

	UNITY_BRANCH
	if (WM_FOG_VOLUMETRIC_LIGHT_MODE_ENABLE_WITH_SHADOWS && WM_CAMERA_RENDER_MODE_NORMAL && _WeatherMakerFogLightShadowBrightness > 0.0 && cloudShadowSquared2 > 0.001 && volumetric && rayLerp > 0.001)
	{
		...
		for (int i = 0; i < int(sampleCount); i++)
		{
			...
			shadowPower += lerp(1.0, UNITY_SAMPLE_SHADOW(_WeatherMakerShadowMapTexture, samplePos), rayLerp);
		}

		// TODO: Refactor into convenience method
		UNITY_BRANCH
		if (rayLerp2 > 0.01 && cloudShadowSquared2 > 0.01)
		{
			// take min distance to volumetric cloud plane if it is hit
			float distanceToPlane;
			if (RayPlaneIntersect(rayOrigin, rayDir, float3(0.0, 1.0, 0.0), float3(weatherMakerCloudCameraPosition.x, _CloudHeightVolumetric, weatherMakerCloudCameraPosition.z), distanceToPlane))
			{
				rayLength = min(rayLength, distanceToPlane);
			}
			fixed cloudShadow = 0.0;
			fixed cloudShadowTerm;
			wpos.xyz = origWpos;
			stepAmount = min(_ProjectionParams.z, rayLength) * invSampleCount;
			stepDir = rayDir * stepAmount;
			stepDir *= dither;
			...
			{
				wpos.xyz += stepDir;
				cloudShadowTerm = lerp(1.0, ComputeCloudShadowStrengthTextureLOD(wpos.xyz, 0, 1.0, false, 1.0), rayLerp2);
				cloudShadow += (cloudShadowTerm);
			}

			shadowPower = min(cloudShadow, shadowPower);
		}
